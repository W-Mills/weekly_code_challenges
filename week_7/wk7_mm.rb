...
# the device will indicate fixed anomalies with stars. The other bad news is 
...
# add the value to starting frequency (plus and minus)

# starting_frequency = 0
# input = File.read('./day1_data.txt')
# values = input.split("\n")
# values.each { |value| starting_frequency += value.to_i }

# puts starting_frequency
...
# Set or Hash for each value reached 
# then can check each subsequent new value against others created.
# loop through the values until a match is found.
require 'Set'

class Frequency_Shift
  attr_accessor :frequency
  attr_reader :values

  def initialize
    @frequency = 0
    input = File.read('./wk7_mm_data.txt')
    @values = input.split("\n")
  end  

  def calculate_frequency
    values.each { |value| self.frequency += value.to_i }
    frequency
  end
  ...
  def calculate_repeat
    ...
    while match_frequency == nil
      values.each do |value| 
        self.frequency += value.to_i 
...
freq = Frequency_Shift.new
puts freq.calculate_frequency
puts freq.calculate_repeat